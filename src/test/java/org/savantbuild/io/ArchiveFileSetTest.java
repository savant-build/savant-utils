...
public class ArchiveFileSetTest extends BaseUnitTest {
  ...
  public void toFileInfosWithPrefix() throws Exception {
    ...
    assertEquals(infos.stream().map((info) -> info.relative).collect(Collectors.toList()), asList(
        ...
        Paths.get("some-directory-1.0/org/savantbuild/util/jar/JarTools.java"),
        Paths.get("some-directory-1.0/org/savantbuild/util/MapBuilder.java"),
        Paths.get("some-directory-1.0/org/savantbuild/util/tar/TarBuilder.java"),
        Paths.get("some-directory-1.0/org/savantbuild/util/tar/TarTools.java"),
        Paths.get("some-directory-1.0/org/savantbuild/util/zip/ZipBuilder.java"),
        Paths.get("some-directory-1.0/org/savantbuild/util/zip/ZipTools.java")
    ));
  }
}
